...
public class ChessGameUI {
				...
				if (aiMove != null) {
					try {
						...
						if (piece == Piece.NONE || type == null) {
							statusLabel.setText("Invalid move: " + aiMove + " (no piece found)");
							statusLabel.setForeground(display.getSystemColor(SWT.COLOR_RED));
							aiThinking = false;
							updateUI();
							return;
						...
						if (computerVsComputer) {
							...
							if (!gameEnded) {
								...
								display.timerExec(ChessConfig.UI.AI_MOVE_DELAY_MS, () -> {
									try {
										if (!shell.isDisposed()) {
											makeAIMove();
										}
	...
	void updateUI() {
		...
		if (computerVsComputer) {
			gameModeLabel.setText("Computer vs Computer (Move: " + board.getMoveCounter() + ")");
		...
		if (aiThinking) {
			if (computerVsComputer) {
				statusLabel.setText("Computer (" + currentSide + ") is thinking...");
			} else {
