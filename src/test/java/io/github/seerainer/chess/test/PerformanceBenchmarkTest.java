...
public class PerformanceBenchmarkTest {
	...
	System.out.println(new StringBuilder().append("  Time: ").append(result.timeMs).append("ms").toString());
	...
	System.out.println(new StringBuilder().append("- Average time per position: ").append("%.1f".formatted(avgTime))
		.append("ms").toString());
	System.out.println(new StringBuilder().append("- Time range: ").append(minTime).append("ms - ").append(maxTime)
		.append("ms").toString());
	...
	// Detailed results
	System.out.println("\nDETAILED RESULTS:");
	results.forEach((final BenchmarkResult result) -> System.out.println(new StringBuilder().append("- ")
		.append(result.positionName).append(": ").append(result.timeMs).append("ms, ").append(result.bestMove)
		.append(", ").append("%.0f".formatted(result.positionsPerSecond)).append(" pos/sec").toString()));
	...
	for (var depth = 4; depth <= 8; depth++) {
	    System.out.println(new StringBuilder().append("\nTesting depth ").append(depth).append("...").toString());
	    benchmarkPosition(
		    new StringBuilder().append("Complex Position (depth ").append(depth).append(")").toString(),
		    complexPosition, depth);
	}
    }
	...
	futures.forEach((final CompletableFuture<BenchmarkResult> future) -> {
		...
		results.add(result);
		System.out.println(new StringBuilder().append("Parallel test completed: ").append(result.positionName)
			.append(" in ").append(result.timeMs).append("ms, move: ").append(result.bestMove).toString());
