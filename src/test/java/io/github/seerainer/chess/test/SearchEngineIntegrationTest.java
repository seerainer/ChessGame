...
public class SearchEngineIntegrationTest {
    ...
    private SearchEngineIntegrationTest() {
	...
	try (final var resourceManager = new ResourceManager()) {
	    ...
	    try (var executorService = resourceManager.getExecutorService()) {
		final var engine = new ParallelSearchEngine(executorService, searchAlgorithms, tt);
		final var board = new Board();
		...
		System.out.println(new StringBuilder().append("Parallel search result: move=").append(result.move)
			.append(", score=").append(result.score).toString());
	    }
